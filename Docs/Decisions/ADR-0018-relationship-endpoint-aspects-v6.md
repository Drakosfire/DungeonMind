...
6. **Kernel vs D&D split.** The kernel reader/scoper owns representation and `effective_endpoint_kind`. The D&D profile owns predicate domain/range validation against the bundled immutable `world-object-v5` catalog. A caller-supplied vocabulary is only an explicit pin request and must match `builtin_world_object_v5_vocabulary_ref()` on vocabulary id, revision, and catalog digest; admission always uses the loader result. The kernel does not import `dungeonmind_dnd`; the adapter does not import Buddy.
...
8. **The #588 adapter emits `DndRelationshipAspectMaterializationPlanV1` only.** It hashes package bytes itself, independently recomputes local admission against the bundled v5 catalog, and does not invent durable aspect assertion IDs or evidence metadata. `source_package_sha256` is exact raw-byte provenance: reordering equivalent package rows yields identical sorted directives but different source hashes and therefore different sealed plan bytes.
